"""Legacy Text Parser Engine

Provides heuristic structural parsing for plaintext IETF RFC documents (RFC 1 - 8649).
Reverse-engineers historical 'nroff' typesetting conventions to extract semantic blocks,
handle pagination, and infer hierarchical structures prior to the adoption of xml2rfc.
"""
...
class LegacyTextParser:
    ...
    def __init__(self, txt_filepath):
        ...
        # Uses errors="ignore" to safely handle historical character encodings
        with open(txt_filepath, encoding="utf-8", errors="ignore") as f:
    ...
    def _extract_blocks(self, clean_text):
        ...
        # Separate standard formatting blocks by double newlines
        ...
        try:
            rfc_int = int(self.rfc_number)
            base_confidence = 0.8 if rfc_int >= 1000 else 0.6
        except ValueError:
            # Fallback confidence for malformed filenames
            base_confidence = 0.5
        ...
        for block in raw_blocks:
            ...
            # Update hierarchy path on header match and skip extraction
            if sec_match or is_unnumbered:
                title = first_line
                if sec_match:
                    # Calculate depth (e.g., "2.1." -> depth 2) to maintain breadcrumbs
            ...
            if is_indented:
                # Detect ABNF grammar via assignment operators
                ...
                # Preserve structural whitespace for diagrams and code
                normalized_text = block
            else:
                block_type = "prose"
                # Compress excessive newlines for standard prose
            ...
            canonical_blocks.append(
                {
                    ...
                    "parsing_confidence": base_confidence,
